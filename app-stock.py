...
window_selection_c.markdown(
    "## Insights")  # add a title to the sidebar container
sub_columns = window_selection_c.columns(
    2)  #Split the container into two columns for start and end date
...
# ---------------stock selection------------------
STOCKS = np.array(["GOOG", "GME", "FB", "AAPL",
                   'TSLA'])  # TODO : include all stocks
SYMB = window_selection_c.selectbox("select stock", STOCKS)

chart_width = st.expander(label="chart width").slider("",
                                                      500,
                                                      2800,
                                                      1000,
                                                      key='CHART_WIDTH')
...
HORIZON = train_test_forecast_c.number_input("Inference horizon",
                                             min_value=7,
                                             max_value=200,
                                             key="HORIZON")
...
train_test_forecast_c.button(label="Train", key='TRAIN_JOB')
