...
class Stock:
    ...
    def load_data(self, start, end, inplace=False):
        ...
        data = yf.download(self.symbol, start,
    ...
    @staticmethod
    def for_prophet(df: pd.DataFrame,
                    date_column="date",
                    y_column="Close") -> pd.DataFrame:
        return df.rename(columns={date_column: "ds", y_column: "y"})

    @st.cache(show_spinner=False)
    def load_train_test_data(self, TEST_INTERVAL_LENGTH,
        ...
        TEST_START = Stock.nearest_business_day(TEST_END - datetime.timedelta(
            days=TEST_INTERVAL_LENGTH))

        TRAIN_END = Stock.nearest_business_day(TEST_START -
    ...
    def plot_inference(self) -> go.Figure:
        ...
        future = self.model.make_future_dataframe(
            periods=st.session_state.HORIZON, include_history=False)
    ...
    def train_test_forecast_report(symb):
        ...
        if st.session_state.TRAIN_JOB or st.session_state.TRAINED:
            text = st.empty(
            )  # Because streamlit adds widgets sequentially, we have to reserve a place at the top (after the chart of part 1)
            ...
            stock.load_train_test_data(
                TEST_INTERVAL_LENGTH, TRAIN_INTERVAL_LENGTH
            ...
            bar.progress(100)
            bar.empty(
            )  #Turn the progress bar object back to what it was before and empty container
            st.markdown(
                f"## {symb} stock forecasts on testing set, Testing error {round(stock.test_mape*100,2)}%"
            )
            st.plotly_chart(fig)
            text.write('Generating forecasts ... ')
            fig2 = stock.plot_inference(
            )  #Generate forecasts and plot them (no cache but figures are not updated if their data didn't change)
            st.markdown(
                f'## Forecasts for the next {st.session_state.HORIZON} days')
